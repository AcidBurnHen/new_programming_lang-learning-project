...
class IfNode:
    def __init__(self, cases, else_case):
        ...
        self.post_start = self.cases[0][0].pos_start
...
class Parser:
    ...
    def advance(self):
        ...
        # print("Self tok idx: ", self.tok_idx)
        # print("Tokens len: ", len(self.tokens))
    ...
    def power(self):
        return self.bin_ops(self.atom, (TT_POW,), self.factor)
    ...
    def term(self):
        # print("Term ran")
        return self.bin_ops(self.factor, (TT_MUL, TT_DIV))

    def arith_expr(self):
        return self.bin_ops(self.term, (TT_PLUS, TT_MINUS))
    ...
    def comp_expr(self):
        ...
        node = res.register(
            self.bin_ops(self.arith_expr, (TT_EE, TT_NE, TT_LT, TT_GT, TT_LTE, TT_GTE))
        )
    ...
    def expr(self):
        ...
        node = res.register(
            self.bin_ops(self.comp_expr, ((TT_KEYWORD, "AND"), (TT_KEYWORD, "OR")))
        )
    ...
    def bin_ops(self, func_a, op_toks, func_b=None):
        # print("Bin ops ran")
        if func_b is None:
            func_b = func_a

        res = ParseResult()
        lnode = res.register(func_a())

        if res.error:
            return res

        # print("Got lnode: ", lnode)
        # print("Current tok type: ", self.current_tok.type)
        while (
            ...
            res.register_advance()
            self.advance()
            rnode = res.register(func_b())
            if res.error:
                return res
            # print("Got rnode: ", rnode)
            lnode = BinOpNode(lnode, op_tok, rnode)

            # print("Got new lnode: ", lnode)

        return res.success(lnode)
...
class Number:
    ...
    def set_pos(self, post_start=None, pos_end=None):
